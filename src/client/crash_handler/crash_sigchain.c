...
void crash_sigchain_register_existing_handler(int sig, const struct sigaction *kernel_old)
{
   if (!crash_sigchain_is_owned(sig)) return;
   crash_disp_lock(sig);
   store_disposition(sig, kernel_old);
...
static inline int crash_maps_check_writable_via_ioctl(int fd,
                                                      uintptr_t addr)
{
