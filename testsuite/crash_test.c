 ...
 * *
 * Crash-cause modes (SIGSEGV; coredump top frame is the crash function):
 ...
 *
 * Abort-message modes (SIGABRT; top frame is glibc raise/abort):
...
int main(int argc, char **argv) {
    ...
    if (strcmp(mode, "all-same") == 0) {
        ...
        /* Does not return — crash_function_A NULL-derefs after the
         * safepoint warmup.  MPI_Finalize is unreachable. */
        return do_safepoint_then_crash(rank);
    } else if (strcmp(mode, "safepoint-bad") == 0) {
        /* Does not return — the read fault hits an unfixed PROT_NONE
         * page, Spindle reports & reraises. */
        return do_safepoint_bad(rank);
    } else if (strcmp(mode, "safepoint-bad-write") == 0) {
        /* Does not return — the write fault hits an unfixed PROT_READ
         * page, Spindle reports & reraises. */
