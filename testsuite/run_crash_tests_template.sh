...
check_prereqs() {
   ...
   if [[ "${cp:0:1}" == "|" ]]; then
      die "crash tests can't run because core_pattern is set to a pipe, preventing coredumps from being produced"
   fi
   if [[ "$cp" != *"%p"* ]]; then
...
main() {
   ...
   if [ -z "$CRASH_TEST_SCRATCH" ]; then
      if [ -n "${SPINDLE_TEST_CONTAINER:-}" ]; then
         die "running in a Spindle test container: --scratch=DIR is required (per-node filesystems are not shared here; ci.yml passes the path for each launcher)"
      fi
